#!/usr/bin/env node
/**
 * ioto-tasks-center 一键发版脚本
 *
 * 用法（在 .obsidian/plugins/ioto-tasks-center 目录下执行）：
 ...
 *   - 只推送本次发版的 tag，绝不使用 `git push --tags`（历史 tag 两端可能指向不同对象）。
 */
...
function printHelp() {
	console.log(`ioto-tasks-center 一键发版
...
async function main() {
	...
	const notesFile = path.join(os.tmpdir(), `ioto-tasks-center-notes-${version}.md`);
